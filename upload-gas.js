...
async function getNewToken(oAuth2Client) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Read OAuth scopes from the manifest file
      const gasDirectory = path.join(process.cwd(), config.google.gasDirectory);
      ScriptManager.readGasManifest(gasDirectory)
        .then(manifest => {
          // Extract OAuth scopes from the manifest
          const scopes = manifest.oauthScopes;
          
          // Generate the authorization URL with scopes from manifest
